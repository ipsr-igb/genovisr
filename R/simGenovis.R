...
simGenoivs <- function(n_sample = 10, 
                       n_marker = 1000,
                       n_chr = 12,
  ...
  out <- list(marker_info = data.frame(chr = chr,
                                       pos= pos,
                                       ref_allele = "G",
                                       alt_allele = "A"),
...
.simGeno4Hap <- function(n_haplotype, n_marker, n_chr){
  ...
  for(i in seq_len(n_haplotype)){
    out[i, ] <- sample(x = c(0, 1), size = ncol(geno4hap), replace = TRUE)
  }
  return(out)
}
...
.getXoPos <- function(i, n_marker, n_chr, n_ploidy){
