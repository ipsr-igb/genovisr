...
```{r}
plotGraphGeno(object = object, direction = "h", data = "haplotype", sample = 1:5, margin = 0.1)
```

```{r}
plotGraphGeno(object = object, direction = "h", data = "dosage", sample = 1:10, margin = 0.1)
```

```{r}
plotGraphGeno(object = object, direction = "v", data = "haplotype", sample = 1, margin = 0.15)
```

```{r}
plotGraphGeno(object = object, direction = "v", data = "dosage", sample = 1, margin = 0.1)
```

The plotly package can be used to make these plots interactive.

```{r}
p <- plotGraphGeno(object = object, direction = "h", data = "haplotype", sample = 1:5, margin = 0.1)
ggplotly(p)
```

```{r}
p <- plotGraphGeno(object = object, direction = "v", data = "haplotype", sample = 1, margin = 0.1)
ggplotly(p)
```
